...
def test_read_transactions_from_csv(mocked_file: Any, transactions_list_for_read_from_csv: Any) -> None:
...
def test_read_transactions_from_csv_exception() -> None:
...
def test_read_transactions_from_xlsx(mocked_df: Any, transactions_from_xlsx: Any) -> None:
    mocked_df.return_value = pd.DataFrame(transactions_from_xlsx["input"])
    assert src.read_data.read_transactions_from_xlsx(os.path.join('data', 'transactions_excel.xlsx')) == \
           transactions_from_xlsx["output"]


def test_read_transactions_from_xlsx_exception() -> None:
    assert src.read_data.read_transactions_from_xlsx('fake_transactions_excel.xlsx') == []
